...
public:
    ...
    void move(double tau) {
        ...
        z += vz * tau;
        t += tau;
        double dist = sqrt((x+10)*(x+10) + (y+10)*(y+10) + (z+10)*(z+10));
        vx = -0.4*dist;
        vy = -0.4*dist;
        vz = -0.3*dist;
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double smth = 0;
            double vx, vy, vz;
            double dist = sqrt(pointX*pointX + pointY*pointY + pointZ*pointZ);
            if (dist > 30 && pointX > 0 && pointY > 0 && pointZ > 0) {
                vx = -0.2*dist;
                vy = -0.2*dist;
                vz = -0.15*dist;
            } else {
...
int main()
{
    ...
    for(unsigned int step = 100; step >=1; --step) {
        mesh.doTimeStep(-tau);
        mesh.snapshot(100 + (100 - step));
    }
