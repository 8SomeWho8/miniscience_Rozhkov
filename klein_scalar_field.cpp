...
public:
    ...
    CalcNode(double x, double y, double z, double smth, double vx, double vy, double vz, double t) 
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double smth = cos(-0.4*(pointX + pointY + pointZ));
            nodes[i] = CalcNode(pointX, pointY, pointZ, smth, 0.0, 0.0, 0.0, 0.0);
        }
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "klein-step-" + std::to_string(snap_number) + ".vtu";
